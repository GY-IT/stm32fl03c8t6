...
for k = N:-1:1
    % 控制增益矩阵 K_k
%     K(:,:,k) =  (B' * P_k * A)\ (R + B' * P_k * B);
%     L(:,:,k) =  (B' * P_k )   \ (R + B' * P_k * B);
    K(:,:,k) =  (R + B' * P_k * B)\(B' * P_k * A);
    Lg(:,:,k) =  -1*(R + B' * P_k * B)\(B'*g_k);
    % 更新 P_k (动态规划递推)
    P_k = Q + A' * P_k * A - A' * P_k * B * K(:,:,k);
    g_k = (-Q*r(:, k) + (A-B*K(:,:,k))'*g_k);
end
...
J = zeros(1, N+1);
J(N+1) = (x(:,N+1)-r(:,N+1))'*QN*(x(:,N+1)-r(:,N+1));
for k = N:-1:1
    J(:,k) = (x(:,k)-r(:,k))'*Q*(x(:,k)-r(:,k)) +u(:,k)'*R*u(:,k) + J(:,k+1);
end
% 绘图
figure;
subplot(4,1,1);
