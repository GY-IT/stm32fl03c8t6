...
%         double_subgrad_ks=zeros(10,1);
        while i 
            i = i + 1 
            subgrad_k = subs(grad_k,Vk(k:num_h),v_vPk);                 % 计算当前梯度
            double_subgrad_k = double(subgrad_k);                       % 计算当前梯度
%             double_subgrad_ks(:,i) = double_subgrad_k;
            v_vPk(1:(num_h-1)) = v_vPk(1:(num_h-1)) - learning_rate * double_subgrad_k; 
            v_vPk(num_h) = 3-(-3)-(ones(1,9)*v_vPk(1:(num_h-1)));
%             vk
            norm(double_subgrad_k)
            if(norm(double_subgrad_k(1)) < 0.1)
                break
            end
             v_vPk'; % 带入梯度公式,更新
        end
