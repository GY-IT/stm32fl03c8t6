...
%         fun_f{:, k} = subs(fun_f{:, k},Vk(1:k-1),vPk(1:k-1)');%赋予之前时刻的速度值给f函数，更新函数
    ...
    v_vPkf = sym('v_vPkf', [num_h 1]);      %包含变量学习率的符号矩阵
    if k ~= num_h
        grad_k = gradient(fun_f{:, k},Vk(1:(num_h-1)));       %对减1个求导

        for i=1:1:20
            subgrad_k = subs(grad_k,Vk(k:num_h),v_vPk);                 % 计算当前梯度
            double_subgrad_k = double(subgrad_k);                       % 计算当前梯度
            double_subgrad_k'
            %求解学习率
            v_vPkf(1:(num_h-1))=v_vPk(1:(num_h-1)) - learning_rate_x * double_subgrad_k;       %前9个包含变量学习率的符号矩阵
            v_vPkf(num_h) = 6-(ones(1,9)*v_vPkf(1:(num_h-1)));                                 %最后1个包含变量学习率的符号矩阵
            learning_rate = vpasolve(diff(subs(fun_f{:, k},Vk(k:num_h),v_vPkf))==0);               %对函数求导=0解学习率
            %一阶导二阶导判断是否为最小值
%             d=diff(subs(fun_f{:, k},Vk(k:num_h),v_vPkf));                 %一阶导
%             double(subs(d,learning_rate_x,learning_rate))                 %一阶导值
%             double(subs(diff(d),learning_rate_x,learning_rate))           %二阶导值
            %学习率范围设置
...
%             end
            learning_rate
            %更新速度
            v_vPk(1:(num_h-1)) = v_vPk(1:(num_h-1)) - learning_rate * double_subgrad_k;     %前9个速度
            v_vPk(num_h) = 6-(ones(1,9)*v_vPk(1:(num_h-1)));                                %最后一个速度
            v_vPk'
...
%     v_vPk_o = v_vPk;                        %将更改后速度储存
    %%%%%%%%%%  P-F 求出此时参考轨迹
    for i = k:num_h
        s1(:,i+1) = s1(i)+v_vPk(i+1-k);        %更改b1 横坐标
    end
    for i = k:num_h_add_1
        p1_P = s1(i);                       %曲线2
