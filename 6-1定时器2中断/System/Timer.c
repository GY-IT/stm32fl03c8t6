...
void Timer_Init(void)
{
	...
	TIM_InternalClockConfig(TIM2);   									//内部时钟给TIM2
	...
	TIM_TimeBaseInitStructure.TIM_Period = 10000-1;						//计数器 有一个数的偏差
	...
	TIM_ITConfig(TIM2, TIM_IT_Update, ENABLE);							//开启中断线路
	
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);						//中断分组
	
	NVIC_InitTypeDef NVIC_InitSturcture;
	NVIC_InitSturcture.NVIC_IRQChannel = TIM2_IRQn;						//选择中断通道
